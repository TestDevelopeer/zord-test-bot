...
try {
	...
	Write-Host "Creating database.sqlite file..." -ForegroundColor Cyan
	docker exec $APP_CONTAINER touch database/database.sqlite
	if ($LASTEXITCODE -ne 0) {
		throw "Error creating database.sqlite file"
	}
